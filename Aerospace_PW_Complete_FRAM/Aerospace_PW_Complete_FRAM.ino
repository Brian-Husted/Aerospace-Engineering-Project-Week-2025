...
void setup() {
  ...
  SerialBT.begin("Your Team Name Here..."); //bluetooth device name for your team
  
  startup(); //play the startup chime

  //wait for devide to connect
...
void loop() {
  ...
  double xyz[3];
  float loop_time;  //time for each loop in seconds
  float vertVel;  //vertical velocity based on altitude m/s
  ...
  //send new data to the log
  telemetry();  //default is: time, state, altitude, and accelX, adjust for orientation of accelerometer
  ...
  switch(state){
    ...
    case 1:  //rocket is ascending
      ...
      loop_time = (millis() - lastTime) / 1000; 
